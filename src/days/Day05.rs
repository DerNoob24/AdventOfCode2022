...
fn task1(input: &[[i32; 3]; 501]) {
    let mut crates = get_crates();
    for i in input {
        for _ in 0..i[0] {
            let source = (i[1] - 1) as usize;
            let target = (i[2] - 1) as usize;
            let c = crates[source].pop().unwrap();
            crates[target].push(c);
        }
    }
    let mut top: Vec<char> = Vec::new();
    for mut i in crates {
        top.push(i.pop().unwrap());
    }
    println!("top: {:?}", top);
}

fn task2(input: &[[i32; 3]; 501]) {
    let mut crates = get_crates();
    for i in input {
        let mut to_move: Vec<char> = Vec::new();
        for _ in 0..i[0] {
            let source = (i[1] - 1) as usize;
            to_move.push(crates[source].pop().unwrap());
        }
        for _ in 0..to_move.len() {
            let target = (i[2] - 1) as usize;
            crates[target].push(to_move.pop().unwrap());
        }
    }
    let mut top: Vec<char> = Vec::new();
    for mut i in crates {
